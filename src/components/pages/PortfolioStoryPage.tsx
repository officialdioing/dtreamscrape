...
export function PortfolioStoryPage({ slug, post: initialPost }: PortfolioStoryPageProps) {
  ...
  // Fetch portfolio item with auto-refresh capability
  const fetchPortfolioItem = useCallback(async () => {
    ...
    try {
      const [itemRes, listRes] = await Promise.all([
        fetch(`/api/portfolio-items/${encodeURIComponent(slug)}?_t=${Date.now()}`, { cache: 'no-store' }),
        fetch(`/api/portfolio-items?_t=${Date.now()}`, { cache: 'no-store' })
      ]);
