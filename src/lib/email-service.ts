...
let serviceStatus = {
  brevo: { available: true, lastError: null as string | null },
  resend: { available: true, lastError: null as string | null },
};
...
// Send email using Brevo (primary service)
async function sendWithBrevo(
  ...
  if (!brevoApiKey || !serviceStatus.brevo.available) {
    return { success: false, error: 'Brevo not available' };
  }

  try {
    const emailPayload = {
      sender: { email: FROM_EMAIL, name: 'Dreamscape Curated Events' },
    ...
    serviceStatus.brevo.available = true;
    serviceStatus.brevo.lastError = null;
    ...
    serviceStatus.brevo.available = false;
    serviceStatus.brevo.lastError = error.message;

    return { success: false, error };
  }
}

// Send email using Resend (backup service)
async function sendWithResend(
  ...
  if (!process.env.RESEND_API_KEY || !serviceStatus.resend.available) {
    return { success: false, error: 'Resend not available' };
  }
  ...
  try {
    ...
    serviceStatus.resend.available = true;
    serviceStatus.resend.lastError = null;
    ...
    serviceStatus.resend.available = false;
    serviceStatus.resend.lastError = error.message;

    return { success: false, error };
  }
}

// Smart email sender with automatic failover
async function sendSmartEmail(
  to: string | string[],
  subject: string,
  htmlContent: string,
  attachments?: Array<{ filename: string; content: string }>
): Promise<{ success: boolean; service: string; data?: any; error?: any }> {

  const toArray = Array.isArray(to) ? to : [to];

  // Try Brevo first (300 emails/day)
  console.log('📧 Attempting to send via Brevo...');
  const brevoResult = await sendWithBrevo(toArray, subject, htmlContent, attachments);

  if (brevoResult.success) {
    return { success: true, service: 'Brevo', data: brevoResult.data };
  }

  // Fallback to Resend (100 emails/day)
  ...
  if (resendResult.success) {
    return { success: true, service: 'Resend', data: resendResult.data };
  }
...
export async function sendCustomerConfirmationEmail(booking: BookingDetails) {
  ...
  return sendSmartEmail(
    booking.email,
    'Consultation Booking Confirmed - Dreamscape Curated Events',
    htmlContent,
    [{ filename: 'consultation-invite.ics', content: icsContent }]
  );
}
...
export async function sendBusinessNotificationEmail(booking: BookingDetails) {
  ...
  return sendSmartEmail(
    BUSINESS_EMAIL,
    `🎉 New Consultation Booking: ${booking.first_name} ${booking.last_name}`,
    htmlContent,
    [{ filename: `${booking.first_name}-${booking.last_name}-consultation.ics`, content: icsContent }]
  );
}
